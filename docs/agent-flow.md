...
6. **Ledger status vocabulary**: every entry's `status` must LEAD with a legal outcome token (`analysis/baseline/closed/correction/diagnostic/instrumentation/measured/promote/reject/retain/retracted/validated`), detail in parentheses after. Run `python3 scripts/check-evidence.py --workspace ../gsim-task-saturate-sparse --vocab-since 2026-08-20` before ending a session — the humanize reviewer flags violations. (2026-09-06: 11 session entries failed the gate on tokens like "REJECTED"/"KILLED"/"measured:" and were repaired in place — format repair of same-day entries is fine; the no-rewrite rule protects EVIDENCE, not vocabulary.)
7. **Tool-enabled consultations**: any consultation that judges CODE (cut-lists, refactor plans, correctness reviews) MUST run tool-enabled (omp print mode with tools + `--auto-approve`, pointed at a stable checkout), never text-only (bare `completion()`). Text-only consultants guess from the brief's framing; the 2026-09-06 dual review proved it: both models voted to cut genMtTaskRunner (which emits the live worker pool core) and the coarse lineage (auto-promoted under the champion recipe) — only the empirical reachability protocol caught them. Text-only remains fine for pure strategy/math questions with all numbers in the brief. Corollary: consultant majority is never evidence; reachability checks are.
